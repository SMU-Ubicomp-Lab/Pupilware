    ...
    public:
        ...
        virtual void fit( const cv::Mat srcGray
            ...
            for (int i = 0; i < maxIteration; ++i) {
                ...
                e[_calSnakeEnergy(srcGray, Point(cPoint.x - 1, cPoint.y), radius, innerRadius)];
                e[_calSnakeEnergy(srcGray, Point(cPoint.x + 1, cPoint.y), radius, innerRadius)];
                e[_calSnakeEnergy(srcGray, Point(cPoint.x, cPoint.y - 1), radius, innerRadius)];
                e[_calSnakeEnergy(srcGray, Point(cPoint.x, cPoint.y + 1), radius, innerRadius)];
                e[_calSnakeEnergy(srcGray, cPoint, radius - 1, _calInnerRadius(radius-1, alpha))];
                e[_calSnakeEnergy(srcGray, cPoint, radius + 1, _calInnerRadius(radius+1, alpha))];
