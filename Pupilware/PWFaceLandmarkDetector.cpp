...
//            }
            if(k>=36 && k <= 41){
                dlib::point p = shape.part(k);
                roiPointsL.push_back(cv::Point(p.x(), p.y()));

            }

            if(k>=42 && k <= 47){
                dlib::point p = shape.part(k);
                roiPointsR.push_back(cv::Point(p.x(), p.y()));
            }
        }
        ...
        cv::Mat maskL = cv::Mat::zeros(out.rows, out.cols, CV_8UC1);
        cv::fillPoly(maskL, ppt, npt, 1, cv::Scalar(255));

        cv::Mat maskR = cv::Mat::zeros(out.rows, out.cols, CV_8UC1);
        cv::fillPoly(maskR, ppt, npt, 2, cv::Scalar(255));

        cv::Mat mTmp = cv::Mat::zeros(out.rows, out.cols, CV_8UC1);
        out.copyTo(mTmp, maskL);

        cv::Mat mTmp2 = cv::Mat::zeros(out.rows, out.cols, CV_8UC1);
        out.copyTo(mTmp2, maskR);
